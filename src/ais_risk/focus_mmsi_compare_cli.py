...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run focus-vs-baseline bundle across user-selected own-ship MMSIs and aggregate by modelset."
    )
    ...
    parser.add_argument("--output-prefix", required=True, help="Output prefix for MMSI-compare summary.")
    parser.add_argument("--focus-own-ship-mmsis", required=True, help="Comma-separated focus own-ship MMSIs.")
    parser.add_argument("--output-root", default="outputs/focus_mmsi_compare", help="Root directory for MMSI runs.")
    ...
    parser.add_argument(
        ...
        default=True,
        help="Reuse baseline sweep summary from first MMSI run for remaining MMSI runs.",
    )
    ...
    print(f"mmsi_rows_csv={summary['mmsi_rows_csv_path']}")
