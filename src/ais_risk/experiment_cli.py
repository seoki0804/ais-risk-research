...
from .experiments import run_baseline_experiment_from_csv, save_experiment_outputs


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run batch baseline experiment over mined own-ship cases.")
    ...
    parser.add_argument("--output-prefix", required=True, help="Prefix for experiment outputs.")
    return parser


def main() -> int:
    args = build_parser().parse_args()
    rows, aggregate = run_baseline_experiment_from_csv(
    ...
    csv_path, json_path = save_experiment_outputs(Path(args.output_prefix), rows, aggregate)
