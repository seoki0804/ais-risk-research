...
class ReviewerQualityAuditGovernanceBridgeAddendumTest(unittest.TestCase):
    def test_includes_multisource_governance_bridge_addendum(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
                ...
                json.dumps(
                    {
                        ...
                        "governed_combined_pass_count": 3,
                        "improved_source_count": 1,
                        "detail_csv_path": str(root / "governance_bridge_detail.csv"),
                    }
                ),
                encoding="utf-8",
            )
            ...
            self.assertTrue(payload["multisource_transfer_governance_bridge_present"])
            self.assertEqual(3, payload["multisource_transfer_governance_bridge"]["source_count"])
            self.assertEqual(1, payload["multisource_transfer_governance_bridge"]["improved_source_count"])
            md_text = Path(summary["summary_md_path"]).read_text(encoding="utf-8")
            self.assertIn("Multi-Source Transfer Governance-Bridge Addendum", md_text)
