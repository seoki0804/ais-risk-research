...
from ais_risk.focus_mmsi_compare import run_focus_mmsi_compare_bundle


class FocusMmsiCompareTest(unittest.TestCase):
    def test_run_focus_mmsi_compare_bundle_generates_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manifest.write_text(
                "\n".join(
                    [
                        "# manifest",
                        "- dataset_id: `sample_focus_mmsi_compare_case_v1`",
                        "| 항목 | 값 |",
                        "|---|---|",
                        "| 해역 | Sample Focus MMSI Compare Area |",
                    ]
                ),
                encoding="utf-8",
            )

            summary = run_focus_mmsi_compare_bundle(
                manifest_path=manifest,
                raw_input_path="examples/sample_ais.csv",
                output_prefix=root / "focus_mmsi_compare",
            ...
            self.assertTrue(Path(summary["mmsi_rows_csv_path"]).exists())
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manifest.write_text(
                "\n".join(
                    [
                        "# manifest",
                        "- dataset_id: `sample_focus_mmsi_compare_no_reuse_case_v1`",
                        "| 항목 | 값 |",
                        "|---|---|",
                        "| 해역 | Sample Focus MMSI Compare No Reuse Area |",
                    ]
                ),
                encoding="utf-8",
            )
            ...
            summary = run_focus_mmsi_compare_bundle(
                manifest_path=manifest,
                raw_input_path="examples/sample_ais.csv",
                output_prefix=root / "focus_mmsi_compare_no_reuse",
                focus_own_ship_mmsis=["440000102", "440000103"],
                benchmark_modelsets=[["rule_score", "logreg"]],
                output_root=root / "outputs",
                run_calibration_eval=False,
                ...
                own_ship_case_eval_min_rows=5,
                own_ship_case_eval_repeat_count=2,
                reuse_baseline_across_mmsis=False,
            )
            self.assertEqual("completed", summary["status"])
            self.assertFalse(bool(summary.get("reuse_baseline_across_mmsis", True)))
    ...
    def test_focus_mmsi_compare_cli_runs(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            manifest = root / "manifest.md"
            manifest.write_text(
                "\n".join(
                    [
                        "# manifest",
                        "- dataset_id: `sample_focus_mmsi_compare_cli_case_v1`",
                        "| 항목 | 값 |",
                        "|---|---|",
                        "| 해역 | Sample Focus MMSI Compare CLI Area |",
                    ]
                ),
                encoding="utf-8",
            )
            ...
            result = subprocess.run(
                [
                    "python",
                    "-m",
                    "ais_risk.focus_mmsi_compare_cli",
                    "--manifest",
                    str(manifest),
                    "--raw-input",
                    "examples/sample_ais.csv",
                    "--output-prefix",
                    str(root / "focus_mmsi_compare_cli"),
                    "--output-root",
                    str(root / "outputs"),
            ...
            summary_json = root / "focus_mmsi_compare_cli_summary.json"
            self.assertTrue(summary_json.exists())
            payload = json.loads(summary_json.read_text(encoding="utf-8"))
            self.assertEqual(["440000102", "440000103"], payload["focus_own_ship_mmsis"])
