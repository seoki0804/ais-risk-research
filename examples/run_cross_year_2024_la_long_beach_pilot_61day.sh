...
DATE="${1:-2024-09-02}"
RUN_DATE="${2:-2026-03-17_r42}"
DATE_TAG="${DATE//-/}"
DEFAULT_BASE="${ROOT}/outputs/2026-03-17_r41_cross_year_2024_candidate_scan/la_long_beach_2024_${DATE_TAG}"
...
OUT_DIR="${ROOT}/outputs/${RUN_DATE}_cross_year_2024_la_long_beach_pilot_${DATE_TAG}"
PREFIX="${OUT_DIR}/la_long_beach_2024_${DATE_TAG}"
FOCUS_DIR="${OUT_DIR}/focus"
DATASET_CSV="${PREFIX}/la_long_beach_2024_pairwise_dataset.csv"
...
Usage:
  ...
  QUALITY_ROWS_OVERRIDE=/path/to/quality_gate_rows.csv \
    run_cross_year_2024_la_long_beach_pilot_61day.sh [DATE] [RUN_DATE]

Description:
  Build a first cross-year LA/Long Beach pairwise pilot from a previously
  converted 2024 raw-like CSV and a quality-gate rows CSV. The wrapper
  automatically selects the top gate-passed MMSIs and runs own_ship/timestamp
  hgbt/logreg benchmarks with the main `0.5 nm` label.
EOF
}
...
(
  ...
  PYTHONPATH=src python -m ais_risk.noaa_focus_pairwise_bundle_cli \
    ...
    --region "la_long_beach_2024|33.4|34.2|-118.6|-117.7|${OWN_MMSIS}"
)

for split in own_ship timestamp; do
  out_prefix="${OUT_DIR}/la_long_beach_2024_${DATE}_${split}"
...
echo "own_ship_summary=${OUT_DIR}/la_long_beach_2024_${DATE}_own_ship_summary.json"
echo "timestamp_summary=${OUT_DIR}/la_long_beach_2024_${DATE}_timestamp_summary.json"
