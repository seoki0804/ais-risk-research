...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize pooled cross-year NY/NJ transfer runs.")
...
def main() -> None:
    ...
    for label in ["ny_nj_2023_to_2024", "ny_nj_2024_to_2023"]:
    ...
    rec_23_24 = next(item for item in records if item["label"] == "ny_nj_2023_to_2024")
    rec_24_23 = next(item for item in records if item["label"] == "ny_nj_2024_to_2023")

    lines = [
        "# 문서명",
        "Cross-Year NY/NJ Transfer Note 61day",
        "",
        "# 문서 목적",
        "`2023 pooled NY/NJ -> 2024 pooled NY/NJ`와 반대 방향 transfer를 한 번에 정리해, pooled cross-year evidence를 true transfer setting까지 확장한다.",
        "",
        "# 대상 독자",
        ...
        "- [확정] current cross-year NY/NJ evidence는 same-day pilot과 pooled own-ship benchmark를 넘어, pooled source-target transfer까지 연결됐다.",
        "",
        "## 2. 요약표",
        "",
    ...
    for record in records:
        direction = "2023->2024" if record["label"] == "ny_nj_2023_to_2024" else "2024->2023"
        lines.append(
    ...
    lines.extend(
        [
            ...
            "",
            "- [확정] current NY/NJ result는 이제 `same-day`, `pooled own-ship`, `pooled cross-year transfer`까지 연결됐다.",
            "- [주의] 다만 이 결과도 여전히 `single harbor` 기준이므로, strongest safe phrasing은 `first cross-year pooled transfer evidence in NY/NJ`다.",
            "",
            "## 4. 진입점",
            "",
            f"- [CSV]({output_csv})",
            f"- [2024 pooled note](/Users/seoki/Desktop/research/outputs/presentation_deck_outline_61day_2026-03-13/cross_year_2024_ny_nj_pooled_note_61day.md)",
        ]
    )
