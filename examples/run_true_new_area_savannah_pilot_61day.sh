...
DATE="${1:-2023-09-07}"
RUN_DATE="${2:-2026-03-17_r23}"
OUT_DIR="${ROOT}/outputs/${RUN_DATE}_true_new_area_savannah_${DATE//-/}"
RAW_CSV="${ROOT}/data/raw/noaa/noaa_us_coastal_all_${DATE}_${DATE}_v1/raw.csv"
BUNDLE_PREFIX="${OUT_DIR}/savannah_true_extension_${DATE}"
FOCUS_DIR="${OUT_DIR}/focus"
DEFAULT_REGION_SPEC="savannah|31.80|32.32|-81.18|-80.78|367726210,538005478,248264000,367108030,636016363,563052100,354712000,636093194,565376000,367082860,271043028,636021155,636021393,636092793"
REGION_SPEC="${REGION_SPEC_OVERRIDE:-${DEFAULT_REGION_SPEC}}"
DATASET_CSV="${BUNDLE_PREFIX}/savannah_pairwise_dataset.csv"

usage() {
  cat <<'EOF'
Usage:
  REGION_SPEC_OVERRIDE='savannah|min_lat|max_lat|min_lon|max_lon|mmsi1,...' \
    run_true_new_area_savannah_pilot_61day.sh [DATE] [RUN_DATE]

Description:
  Build a reviewer-grade brand-new-area Savannah NOAA pairwise pilot bundle
...
for split in own_ship timestamp; do
  out_prefix="${OUT_DIR}/savannah_${DATE}_${split}"
...
echo "own_ship_summary=${OUT_DIR}/savannah_${DATE}_own_ship_summary.json"
echo "timestamp_summary=${OUT_DIR}/savannah_${DATE}_timestamp_summary.json"
