...
DATE="${1:-2024-09-05}"
RUN_DATE="${2:-2026-03-17_r28}"
DATE_TAG="${DATE//-/}"
DEFAULT_BASE="${ROOT}/outputs/2026-03-17_r27_expand_cross_year_2024_candidate_scan/ny_nj_2024_${DATE_TAG}"
...
OUT_DIR="${ROOT}/outputs/${RUN_DATE}_cross_year_2024_ny_nj_pilot_${DATE_TAG}"
PREFIX="${OUT_DIR}/ny_nj_2024_${DATE_TAG}"
FOCUS_DIR="${OUT_DIR}/focus"
DATASET_CSV="${PREFIX}/ny_nj_2024_pairwise_dataset.csv"
...
Usage:
  ...
  QUALITY_ROWS_OVERRIDE=/path/to/quality_gate_rows.csv \
    run_cross_year_2024_ny_nj_pilot_61day.sh [DATE] [RUN_DATE]

Description:
  Build a first cross-year NY/NJ pairwise pilot from a previously converted
...
(
  ...
  PYTHONPATH=src python -m ais_risk.noaa_focus_pairwise_bundle_cli \
    ...
    --region "ny_nj_2024|40.3|41.1|-74.5|-73.5|${OWN_MMSIS}"
)

for split in own_ship timestamp; do
  out_prefix="${OUT_DIR}/ny_nj_2024_${DATE}_${split}"
...
echo "own_ship_summary=${OUT_DIR}/ny_nj_2024_${DATE}_own_ship_summary.json"
echo "timestamp_summary=${OUT_DIR}/ny_nj_2024_${DATE}_timestamp_summary.json"
